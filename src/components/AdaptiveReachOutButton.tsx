...
export const AdaptiveReachOutButton = ({
  ...
  // If skipRevealAnimation is true, we use opacity-0 to keep it hidden initially
  // allowing external logic (like Landing.tsx) to control it.
  // Otherwise, we use .reveal which is also opacity 0 but gets animated by usePageTransition.
  const containerClass = skipRevealAnimation
    ? `opacity-0 ${className || ""}`
    : `reveal ${className || ""}`;
  ...
  return (
    ...
    >
      <div
        ...
        style={skipRevealAnimation ? { opacity: 0 } : undefined}
      >
        <div
          style={{
        ...
        >
          <SmartButton
            ...
            backgroundColor={
              isDarkBg ? "var(--parchment)" : "var(--charcoal-blue)"
            }
