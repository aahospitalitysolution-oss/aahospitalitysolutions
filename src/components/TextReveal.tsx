...
export function TextReveal({
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const spanEls = wordEls.map(
        (el) => el.querySelector("span") as HTMLSpanElement
      );
      ...
      const toRgbString = (r: number, g: number, b: number, a = 1) =>
        `rgba(${clamp(r)}, ${clamp(g)}, ${clamp(b)}, ${clamp(a, 0, 1)})`;
      ...
      const findBackgroundRgb = (
        ...
        return bodyRgb
          ? { r: bodyRgb.r, g: bodyRgb.g, b: bodyRgb.b }
          : { r: 255, g: 255, b: 255 };
      };
      ...
      const l2 = isLight
        ? Math.max(0, l - lightnessDelta)
        : Math.min(1, l + lightnessDelta);
      ...
      gsap.set(wordEls, {
        opacity: 0,
        ...(variant === "default" ? { backgroundColor: pillColor } : {}),
      });
      ...
      const overlapWords = Math.max(
        6,
        Math.min(15, Math.round(totalWords * 0.4))
      );
      ...
      wordEls.forEach((wordEl, index) => {
        ...
        if (variant === "default") {
          ...
          tl.to(
            wordEl,
            {
              backgroundColor: pillTransparent,
              duration: windowDuration * 0.5,
              ease: "sine.inOut",
            },
  ...
  return (
    <div
      ref={containerRef}
      className={`${styles.animeText} ${sizeClass} ${
